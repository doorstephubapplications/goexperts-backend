...
// I want to inject the enrichment logic at the end of the file, and then call it from listClientProjects and getClientProject.
...
// Update listClientProjects
const listMapSearch = `    const enrichedRows = rows.map(r => {
...
const listMapReplace = `    let enrichedRows = rows.map(r => {
      const br = budgetRanges.find(b => b.id === r.budgetRangeId);
      return {
        ...r,
        budgetRange: br ? { id: br.id, label: br.label, min: br.min, max: br.max, value: br.value } : null
...
content = content.replace(listMapSearch, listMapReplace);

// Update getClientProject
const getReturnSearch = `    res.json({ success: true, data: { ...project, budgetRange, tasks, proposals, contracts } });`;
...
content = content.replace(getReturnSearch, getReturnReplace);

fs.writeFileSync(file, content);
console.log('client.controller.ts patched');
