...
target = r"""export const rejectInvitation = async \(req: AuthRequest, res: Response, next: NextFunction\) => \{[\s\S]*?return res\.json\(successResponse\('Invitation rejected', updated\)\);\s*\} catch \(error\) \{ next\(error\); \}\s*\};"""
