...
async function buildMonthlyRevenue() {
    ...
    for (const key of lastNMonthKeys())
        buckets.set(key, { revenue: 0, subscriptions: 0 });
    for (const payment of payments) {
        ...
        if (payment.subscriptionId)
            bucket.subscriptions += Number(payment.amount) || 0;
    }
...
export class InsightsService {
    static async getDashboardOverview() {
        ...
        const [totalUsers, freelancers, clients, investors, founders, activeProjects, allProjects, pendingApprovals, subscriptions, activeTasks, openTickets, revenueAgg, prevRevenueAgg, recentUsers, recentPayments, revenueTrend, userGrowth, categories, avgBudget,] = await Promise.all([
            prisma.user.count(),
    ...
    static buildTrafficSeries(apiLogs, registrationTrend) {
        if (apiLogs.length > 0) {
            const dayBuckets = new Map();
            for (const log of apiLogs) {
                const day = `D${log.createdAt.getDate()}`;
                if (!dayBuckets.has(day))
                    dayBuckets.set(day, { organic: 0, paid: 0, referral: 0 });
                const bucket = dayBuckets.get(day);
                if (log.url?.includes("campaign") || log.url?.includes("ads"))
                    bucket.paid += 1;
                else if (log.url?.includes("ref"))
                    bucket.referral += 1;
                else
                    bucket.organic += 1;
            }
    ...
    static buildTrafficSources(countryDistribution, categoryDistribution) {
    ...
    static buildConversionSeries(revenueTrend) {
