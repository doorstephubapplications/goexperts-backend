...
export const getLimitByRole = (role) => {
    switch (role) {
        case "superadmin":
...
export const dynamicRateLimiter = (options = {}) => {
    return rateLimit({
        ...
        max: async (req) => {
            ...
            const role = req.apiKeyDetails?.roleMapping || req.user?.role;
        ...
        handler: (req, res) => {
            return sendResponse(res, 429, false, "Too many requests. Please try again later.", undefined, {
                errors: {
