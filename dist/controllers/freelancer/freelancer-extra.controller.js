    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (project.client === userId)
                throw new Error("Cannot bid on your own project");
            ...
            if (existing)
                throw new Error("You already have an active proposal for this project.");
            // 3. Create Proposal
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (inv.status !== "PENDING")
                throw new Error(`Cannot respond to an invitation in ${inv.status} state`);
...
export const listFreelancerInvitations = async (req, res, next) => {
    try {
        const userId = req.user?.id || req.userId;
        if (!userId)
            return res.status(401).json({ success: false });
