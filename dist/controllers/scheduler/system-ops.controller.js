...
export const generateReportImmediately = async (req, res, next) => {
    try {
...
export const getBackupsList = async (req, res, next) => {
    try {
...
// ============================================================
export const getDashboardSchedulerStats = async (req, res, next) => {
    try {
        const now = new Date();
        const [totalJobs, activeJobs, pausedJobs, failedJobs, upcomingJobs, rulesCount, recentHistory, queueStats] = await Promise.all([
        ...
        const queueMap = queueStats.reduce((acc, curr) => {
