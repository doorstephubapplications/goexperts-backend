...
function canReuseRegistrationEmail(user) {
    if (!user)
        return true;
    if (user.deletedAt)
        return true;
    const status = String(user.status || "").trim().toLowerCase();
    if (["pending_deletion", "deleted", "deleted_by_user"].includes(status))
        return true;
...
export const saveOnboardingDraft = async (req, res, next) => {
    try {
        ...
        const { step, bio, phone, country, countryId, state, stateId, city, cityId, 
        // Freelancer fields
        ...
        if (bio !== undefined)
            userUpdate.bio = getStringVal(bio);
        if (phone !== undefined)
            userUpdate.phone = getStringVal(phone);
        const finalCountry = countryId ?? country;
        const finalState = stateId ?? state;
        const finalCity = cityId ?? city;
        if (finalCountry !== undefined)
            userUpdate.country = getStringVal(finalCountry);
        if (finalState !== undefined)
            userUpdate.state = getStringVal(finalState);
        if (finalCity !== undefined)
            userUpdate.city = getStringVal(finalCity);
        await prisma.user.update({
            where: { id: userId },
            data: userUpdate,
        });
...
export const checkEmailVerification = async (req, res, next) => {
    try {
        ...
        if (!email)
            return res.json({ success: true, message: 'Verified', data: { verified: false } });
        const user = await prisma.user.findUnique({ where: { email: String(email).trim().toLowerCase() } });
        if (!user)
            return res.json({ success: true, message: 'User not found', data: { verified: false } });
