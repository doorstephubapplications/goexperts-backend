...
export const listClientMeetings = async (req, res, next) => {
    try {
        ...
        const userRefs = Array.from(new Set(rows.flatMap((meeting) => [meeting.founder, meeting.investor]).filter((value) => UUID_RE.test(String(value || "")))));
...
export const resendClientTeamInvite = async (req, res, next) => {
    try {
        const userId = requireUser(req, res);
        if (!userId)
            return;
        ...
        let member = null;
        if (memberId) {
            member = await prisma.clientTeamMember.findFirst({
                where: { id: memberId }
            });
        }
        if (!member && reqEmail) {
...
export const getClientProposal = async (req, res, next) => {
    try {
        const userId = req.user?.id || req.userId;
        if (!userId)
            return res.status(401).json({ success: false });
...
export const updateClientTeamMember = async (req, res, next) => {
    try {
        ...
        let permissionsString = undefined;
        if (permittedDashboards || modulePermissions !== undefined) {
            ...
            try {
                if (current?.permissions)
                    currentPerms = JSON.parse(current.permissions);
            }
            catch (e) { }
...
async function enrichProjects(projects) {
    ...
    projects.forEach(r => {
        ...
        if (r.client && uuidRegex.test(r.client))
            allIds.add(r.client);
    });
    const moMap = new Map();
    if (allIds.size > 0) {
        ...
        moRes?.forEach((mo) => moMap.set(mo.id, mo.label || mo.value));
        userRes?.forEach((u) => moMap.set(u.id, u.fullName));
        industryRes?.forEach((i) => moMap.set(i.id, i.name));
        workModeRes?.forEach((w) => moMap.set(w.id, w.name));
        skillRes?.forEach((s) => moMap.set(s.id, s.name));
        categoryRes?.forEach((c) => moMap.set(c.id, c.name));
    }
    const mapVal = (val) => {
        if (typeof val !== 'string')
            return val;
        return val.split(',').map(s => {
            ...
            if (t === 'mo_experience_level_intermediate')
                return 'Intermediate';
            if (t === 'mo_experience_level_expert')
                return 'Expert';
            if (t === 'mo_experience_level_beginner')
                return 'Beginner';
