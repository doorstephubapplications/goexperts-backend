...
export class SystemHealthService {
    ...
    // ============================================================
    static async getApiLogs(page = 1, limit = 50, method, statusCode) {
    ...
    static async getDatabaseStats() {
        ...
        const [users, projects, proposals, contracts, reviews, startups, investments, meetings, subscriptions, payments, invoices, wallets, coupons, referrals, supportTickets, notifications, campaigns, scheduledJobs, automationRules, cronExecutions, auditLogs, activityLogs, apiRequestLogs, loginAttempts, systemAlerts, backups, mediaFiles, adminUsers,] = await Promise.all([
            prisma.user.count(),
    ...
    static async getSecurityOverview() {
        ...
        const [totalAttempts, successfulLogins, failedLogins, attemptsLast24h, failedLast24h, activeSessions, expiredSessions, revokedSessions, recentAuditLogs, recentActivityLogs,] = await Promise.all([
            prisma.loginAttempt.count(),
    ...
    static async getLoginAttempts(page = 1, limit = 50, success) {
        const skip = (page - 1) * limit;
        const where = {};
    ...
    static async revokeSession(sessionId, actorId) {
    ...
    static async getOperationsDashboard() {
        ...
        const [totalApiRequests1h, apiErrors1h, activeJobs, failedJobs, pendingQueue, failedQueue, activeSessions, failedLoginsLast1h, activeAlerts, criticalAlerts, lastBackup, dbCheck,] = await Promise.all([
    ...
    // ============================================================
    static async getAlerts(status, severity, page = 1, limit = 50) {
        const skip = (page - 1) * limit;
        const where = {};
        if (status)
            where.status = status;
    ...
    static async acknowledgeAlert(id, actorId) {
    ...
    static async resolveAlert(id, actorId) {
        const alert = await prisma.systemAlert.update({
            where: { id },
            data: {
                status: "resolved",
