...
export class NotificationService {
    static emailAdapter = new EmailChannelAdapter();
    static smsAdapter = new SmsChannelAdapter();
    static whatsappAdapter = new WhatsAppChannelAdapter();
    static pushAdapter = new PushChannelAdapter();
    static inAppAdapter = new InAppChannelAdapter();
    /**
     * Render subject/body with key-value placeholders
     */
    static renderTemplateText(text, variables) {
        let rendered = text;
    ...
    static async enqueue(params) {
        ...
        if (templateCode) {
            ...
            if (template) {
                if (template.subject)
                    finalTitle = this.renderTemplateText(template.subject, variables || {});
     ...
     */
    static getAdapterForChannel(channel) {
        switch (channel) {
            case "email":
    ...
    static async processQueueItem(queueId) {
        ...
        if (notif.userId) {
            ...
            if (pref) {
                if (notif.channel === "email" && !pref.emailEnabled)
                    isOptedIn = false;
                if (notif.channel === "push" && !pref.pushEnabled)
                    isOptedIn = false;
                if (notif.channel === "sms" && !pref.smsEnabled)
                    isOptedIn = false;
                if (notif.channel === "whatsapp" && !pref.whatsappEnabled)
                    isOptedIn = false;
                if (notif.channel === "in_app" && !pref.inAppEnabled)
                    isOptedIn = false;
     ...
     */
    static async retryQueueItem(queueId) {
     ...
     * Cancel a queue item
     */
    static async cancelQueueItem(queueId) {
        return prisma.$transaction(async (tx) => {
            const queue = await tx.notificationQueue.findUnique({
                where: { id: queueId },
                include: { notification: true },
