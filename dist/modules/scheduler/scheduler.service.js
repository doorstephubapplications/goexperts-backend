...
export class SchedulerService {
     ...
     */
    static async pauseJob(id, updatedBy = "system") {
     ...
     */
    static async resumeJob(id, updatedBy = "system") {
        const job = await prisma.scheduledJob.findUnique({ where: { id } });
        if (!job)
            throw new Error("Job not found");
     ...
     */
    static async cancelJob(id, updatedBy = "system") {
        const job = await prisma.scheduledJob.findUnique({ where: { id } });
        if (!job)
            throw new Error("Job not found");
        const updated = await prisma.scheduledJob.update({
     ...
     */
    static async runImmediately(id) {
        const job = await prisma.scheduledJob.findUnique({ where: { id } });
        if (!job)
            throw new Error("Job not found");
     ...
     */
    static calculateNextRun(type, cronExpression, timezone) {
        if (type === "one_time")
            return null;
