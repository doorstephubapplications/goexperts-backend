export type CatalogListBody = {
  page?: number;
  pageSize?: number;
  search?: string;
};

export type SkillsListBody = CatalogListBody & {
  categoryId?: string;
  industryId?: string;
};

export type FreelancersListBody = CatalogListBody & {
  categoryId?: string;
  industryId?: string;
  experience?: string[];
  rateMin?: number;
...
export function parseCatalogListBody(body: Record<string, unknown> = {}): CatalogListBody {
...
export function parseSkillsListBody(body: Record<string, unknown> = {}): SkillsListBody {
...
export function parseFreelancersListBody(body: Record<string, unknown> = {}): FreelancersListBody {
  const base = parseCatalogListBody(body);
  const categoryId =
    (typeof body.categoryId === "string" && body.categoryId.trim()) ||
    (typeof body.industryId === "string" && body.industryId.trim()) ||
