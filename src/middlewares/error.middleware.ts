...
function getFriendlyErrorMessage(err: Error | ApiError, statusCode: number) {
  if (err instanceof ApiError) return err.message || STATUS_MESSAGES[statusCode];

  const message = err.message || "";
  const missingField = message.match(/Argument `([^`]+)` is missing/i);
  if (missingField?.[1]) return `Missing required field: ${missingField[1]}.`;
  ...
  if (code === "P2002" || /Unique constraint failed/i.test(message)) return "A record with these details already exists.";
  ...
  if (code === "P2003" || /Foreign key constraint/i.test(message)) return "Please select valid related records before saving.";
  if (code === "P2025") return "The requested record could not be found.";

  const unknownColumn =
    message.match(/Unknown column [`'"]([^`'"]+)[`'"]/i)?.[1] ||
  ...
  if (statusCode >= 500) return STATUS_MESSAGES[statusCode] ?? STATUS_MESSAGES[500];
...
function getErrorStatusCode(err: Error | ApiError) {
  ...
  const code = (err as any).code;
  const message = err.message || "";
  if (code === "P2002" || /Unique constraint failed/i.test(message)) return 409;
