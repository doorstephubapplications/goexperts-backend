...
export async function renewSubscription(req: Request, res: Response) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!sub || !sub.plan) throw new Error("Subscription not found");
...
export async function upgradeSubscription(req: Request, res: Response) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!newPlan || newPlan.status !== "active") throw new Error("New plan not available");
...
export async function addWalletBonus(req: Request, res: Response) {
  try {
...
export async function listPayments(req: Request, res: Response) {
  try {
    ...
    res.json({ success: true, data: payments, pagination: { total, page: parseInt(page as string), limit: parseInt(limit as string), pages: Math.ceil(total / parseInt(limit as string)) } });
  } catch (e: any) {
    res.status(500).json({ success: false, message: e.message });
  }
}
...
export async function processRefund(req: Request, res: Response) {
  try {
    ...
    const paymentLookup = await prisma.payment.findUnique({ where: { id: paymentId } });
    if (!paymentLookup) return res.status(404).json({ success: false, message: "Payment not found" });
    if (paymentLookup.status === "refunded") {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const refund = await tx.paymentRefund.create({
        data: {
          paymentId,
          amount: refundAmount,
          reason: `${reason || "Admin initiated refund"}${gatewayRefundId ? ` | gatewayRefundId=${gatewayRefundId}` : ""}`,
          status: "processed",
...
export async function validateCoupon(req: Request, res: Response) {
  try {
    ...
    if (coupon.status !== "active") return res.status(400).json({ success: false, message: "Coupon inactive" });
    if (coupon.uses >= coupon.maxUses) return res.status(400).json({ success: false, message: "Coupon usage limit reached" });
    ...
    if (userId) {
      ...
      if (used) return res.status(400).json({ success: false, message: "Coupon already used by this user" });
    }
...
export async function listCouponUsages(req: Request, res: Response) {
  try {
...
export async function createReferral(req: Request, res: Response) {
  try {
    ...
    if (existing) return res.status(400).json({ success: false, message: "Referee already has a referral" });
...
export async function listAdPlans(req: Request, res: Response) {
  try {
...
export async function createAdPlan(req: Request, res: Response) {
  try {
...
export async function rejectAd(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const ad = await prisma.advertisement.update({
      where: { id },
...
export async function listFeaturedServices(req: Request, res: Response) {
  try {
    const { status } = req.query;
    const where: any = {};
    if (status) where.status = status;
...
export async function createFeaturedService(req: Request, res: Response) {
  try {
...
export async function getFinancialDashboard(req: Request, res: Response) {
  try {
    ...
    const [
      ...
      totalWalletBalance,
      activeCoupons, totalCouponUsages,
      totalReferrals, rewardedReferrals,
      activeAds, totalFeatured,
      totalInvoices, paidInvoices,
      revenueByPlan, revenueByGateway,
    ...
    for (const sub of revenueByPlan) {
      ...
      if (!planRevenue[planName]) planRevenue[planName] = { count: 0, revenue: 0 };
