...
// ============================================================
export const generateApiKey = async (req: any, res: Response, next: NextFunction) => {
  try {
...
export const revokeApiKey = async (req: any, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
export const regenerateApiKey = async (req: any, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
// ============================================================
export const createWebhook = async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    const webhook = await prisma.webhook.create({
      data: {
        ...
        secret,
        events: {
          create: events.map((e: string) => ({ event: e })),
        },
      },
      include: { events: true },
    });
...
export const listWebhooks = async (req: any, res: Response, next: NextFunction) => {
  try {
...
export const updateWebhook = async (req: any, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
export const deleteWebhook = async (req: any, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
export const testWebhook = async (req: any, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    const success = await WebhookService.dispatchWebhook(
      hook.id,
      hook.url,
      hook.secret,
      "test.ping",
    ...
    return sendResponse(
      ...
      success ? 200 : 502,
      success,
      success ? "Webhook verification ping sent successfully." : "Webhook ping failed to respond successfully."
    );
  } catch (err) {
    next(err);
  }
...
export const listWebhookDeliveries = async (req: any, res: Response, next: NextFunction) => {
  try {
...
export const replayWebhookDelivery = async (req: any, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    return sendResponse(
      ...
      success ? 200 : 502,
      success,
      success ? "Webhook delivery replayed successfully." : "Replayed webhook response returned an error status."
    );
  } catch (err) {
    next(err);
  }
...
// ============================================================
export const getDeveloperDashboard = async (req: any, res: Response, next: NextFunction) => {
  try {
    const now = new Date();
    ...
    return sendResponse(res, 200, true, "Developer dashboard fetched successfully.", {
      metrics: {
        ...
        apiKeysCount,
        webhookSuccessRate:
          webhookSuccessCount + webhookFailCount > 0
